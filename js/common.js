$(function () {
	...
	$('.gallery-tizer-wrap')
		...
		.css(
			'background-image',
			'url(' + $('.gallery-images a').data('preview') + ')',
		)
	$('.gallery-images a:first').remove()

	$('.certificate-wrap > a').attr('href', $('.certificates a').attr('href'))
	...
	$('.main-foot .toggle-mnu').click(function () {
		$('html, body').animate({ scrollTop: $(document).height() + 2000 }, 'slow')
		return false
	})
	...
	$('.main-mnu ul li a').click(function () {
		// Закрыть меню
		$('.toggle-mnu').removeClass('on')
		if ($(window).width() < 992) {$('.main-mnu').css('display', 'none')}
